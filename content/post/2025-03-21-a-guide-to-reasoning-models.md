...
P.S. Given the API costs, I replaced all ChatGPT examples with DeepSeek models in my actual experiments. 😂

## Introduction to OpenAI's o1 Models

o1是OpenAI在2024年推出的新型推理模型系列。它最大的特点在于：在给出答案之前，会先进行深入的思考。正是这种特性让它在处理复杂任务时能够达到前所未有的表现水平。
...
O1 模型最擅长的一个应用场景是制定解决任务的计划，在这个过程中，我们有一组工具来执行计划，并且有约束条件来限定任务的范围。

但是，如果在任务的每一个步骤都使用 O1 模型来执行，那整个过程就会变得非常慢。因此，一个更好的做法是用 O1-mini 生成计划，然后用 GPT-4o-mini 执行每个具体步骤。

这种方法实际上是在智能性、延迟和成本之间做权衡。这种策略已经被开发者广泛使用，并且效果很好。

关键点解析：
	•	O1-mini 负责制定计划，因为它更适合高效地规划任务。
	•	GPT-4o-mini 负责执行计划的各个步骤，这样可以减少延迟并降低成本。
